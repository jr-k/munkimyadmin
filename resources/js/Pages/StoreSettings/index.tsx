...
type StoreSettingsProps = PageProps & {
...
export default function StoreSettings({ settings, publicStoreUrl }: StoreSettingsProps) {
    const { props } = usePage<StoreSettingsProps>();
    const { t } = useI18n();
    ...
    const storeInitial = form.data.name.trim().slice(0, 1).toUpperCase() || 'S';
    ...
    return (
        <>
            <Head title={t('store.settingsTitle')} />
            <AppLayout adminEmail={adminEmail}>
                ...
                <S.PageHeader>
                    <div>
                        <S.Eyebrow>{t('common.store')}</S.Eyebrow>
                        <S.Title>{t('store.settingsTitle')}</S.Title>
                        <S.Subtitle>{t('store.settingsSubtitle')}</S.Subtitle>
                    </div>
                </S.PageHeader>
                ...
                <S.SettingsStack>
                    <S.PreviewPanel>
                        ...
                        <S.PreviewCard $mainColor={form.data.main_color}>
                            <S.PreviewLogo $mainColor={form.data.main_color}>
                                {logoPreview ? <S.PreviewLogoImage alt="" src={logoPreview} /> : storeInitial}
                            </S.PreviewLogo>
                            <S.PreviewText>
                                <S.PreviewTitle>{form.data.name || t('store.title')}</S.PreviewTitle>
                                <S.PreviewSubtitle>{t('store.previewSubtitle')}</S.PreviewSubtitle>
                            </S.PreviewText>
                            <S.PublicLink href={publicStoreUrl} target="_blank" rel="noreferrer">
                                {t('store.openPublicStore')}
                            </S.PublicLink>
                        </S.PreviewCard>
                    </S.PreviewPanel>
                    ...
                    <S.Panel>
                        ...
                        <S.Form onSubmit={submit}>
                            <FormField label={t('store.nameLabel')} error={form.errors.name}>
                                <S.Input
                                    value={form.data.name}
                                    onChange={(event) => form.setData('name', event.target.value)}
                                    placeholder={t('store.title')}
                                />
                            </FormField>
